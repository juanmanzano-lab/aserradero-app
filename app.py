    ...
    for p1_seq, h_p1 in p1_candidates[:30]:
        ...
        for p2_seq in p2_candidates:
            ...
            for idx, t in enumerate(p1_seq):
                ...
                piezas_cinta.append({"id": f"F1-P{idx+1}", "origen": "Fase 1", "espesor": t, "w_min": w_min, "cota_z": round(z_cut_bancada, 2)})
            ...
            for idx_rev, t in enumerate(reversed(p2_seq)):
                ...
                is_base = (idx_rev == 0)
                piezas_cinta.append({"id": f"F2-P{len(p2_seq)-idx_rev}", "origen": "Fase 2", "espesor": t, "w_min": w_min, "cota_z": round(z_bot, 2)})
            ...
            for p in piezas_cinta:
                ...
                if abs(t - 2.0) < 0.1:
                    ...
                    detalles_fase3.append({
                        "Pieza": p["id"], "Espesor Primario": f"{t:.1f} cm", "Ancho Útil": f"{w_min:.1f} cm",
                        "Procesamiento": "Canteado a 7 y 9 cm",
                        "Resultado": f"{n7} pzs (2x7) + {n9} pzs (2x9)"
                    })
                    esquema_piezas_fase3.append({"id": p["id"], "espesor": t, "w_min": w_min, "n7": n7, "n9": n9, "n_capas": 1, "tipo": "2cm"})
                    ...
                    detalles_fase3.append({
                        "Pieza": p["id"], "Espesor Primario": f"{t:.1f} cm", "Ancho Útil": f"{w_min:.1f} cm",
                        "Procesamiento": "Canteado a 7 y 9 cm",
                        "Resultado": f"{n7} pzs (4x7) + {n9} pzs (4x9)"
                    })
                    esquema_piezas_fase3.append({"id": p["id"], "espesor": t, "w_min": w_min, "n7": n7, "n9": n9, "n_capas": 1, "tipo": "4cm"})
                    ...
                    cnt_2x9 += n_capas * n9
                    detalles_fase3.append({
                        "Pieza": p["id"], "Espesor Primario": f"Bloque {t:.1f} cm", "Ancho Útil": f"{w_min:.1f} cm",
                        "Procesamiento": f"Re-aserrado a {n_capas} capas x 2cm",
                        "Resultado": f"{n_capas*n7} pzs (2x7) + {n_capas*n9} pzs (2x9)"
                    })
                    esquema_piezas_fase3.append({"id": p["id"], "espesor": t, "w_min": w_min, "n7": n7, "n9": n9, "n_capas": n_capas, "tipo": "bloque"})
            ...
            if score > best_score:
                ...
                best_sol = {
                    ...
                    "esquema_piezas_fase3": esquema_piezas_fase3,
...
# --- GRÁFICOS SIERRA CINTA (FASES 1 Y 2) ---
...
# --- DIBUJO GRÁFICO DE FASE 3 (PIEZAS EFECTIVAS EN SIERRA MÚLTIPLE) ---
def generar_grafico_fase3_multiples(sol):
    esquema = sol["esquema_piezas_fase3"]
    kc_m = sol["kc_multi"]
    n_piezas = len(esquema)

    fig, axes = plt.subplots(n_piezas, 1, figsize=(13, max(2.5 * n_piezas, 5)))
    if n_piezas == 1:
        axes = [axes]
    ...
    for idx, item in enumerate(esquema):
        ax = axes[idx]
        w_min = item["w_min"]
        t = item["espesor"]
        n7 = item["n7"]
        n9 = item["n9"]
        n_capas = item["n_capas"]
        p_id = item["id"]

        ax.set_title(f"Pieza Primaria {p_id} (Espesor: {t:.1f} cm | Ancho Útil: {w_min:.1f} cm)", fontsize=10, fontweight='bold', loc='left')

        # Dibuja el marco total del ancho útil disponible
        rect_total = patches.Rectangle((0, 0), max(w_min, 10.0), t, edgecolor='#2C3E50', facecolor='#EAEDED', hatch='//', alpha=0.5)
        ax.add_patch(rect_total)

        if w_min < 7.0:
            ax.text(w_min/2.0 if w_min > 0 else 5.0, t/2.0, "DESPERDICIO / RETAL (Ancho < 7 cm)", color='#922B21', fontweight='bold', fontsize=9, ha='center', va='center')
        else:
            x_curr = 0.0
            
            # Anchos a colocar: primero anchos de 9cm, luego de 7cm (o según combinación)
            anchos = ([9.0] * n9) + ([7.0] * n7)
            
            # Dibujar capas si es bloque
            h_capa = 2.0 if item["tipo"] == "bloque" else t

            for c_idx in range(n_capas):
                y_capa = c_idx * (h_capa + kc_m)
                x_curr = 0.0

                for w_p in anchos:
                    label_key = f"{int(h_capa)}x{int(w_p)}"
                    c_fill = color_map.get(label_key, "#27AE60")

                    # Rectángulo de la pieza efectiva corta
                    rect_p = patches.Rectangle((x_curr, y_capa), w_p, h_capa, edgecolor='#1B2631', facecolor=c_fill, alpha=0.9)
                    ax.add_patch(rect_p)

                    # Etiqueta de la pieza
                    ax.text(x_curr + w_p/2.0, y_capa + h_capa/2.0, f"{label_key}\n({h_capa:.0f}x{w_p:.0f} cm)", color='white', fontweight='bold', fontsize=8, ha='center', va='center')

                    # Línea de kerf circular
                    x_curr += w_p
                    if x_curr < w_min:
                        ax.axvline(x_curr, color='#E74C3C', linestyle=':', linewidth=1.0)
                        x_curr += kc_m

            # Zona sobrante de costero/retal lateral
            if x_curr < w_min:
                w_rest = w_min - x_curr
                ax.text(x_curr + w_rest/2.0, t/2.0, f"Retal\n{w_rest:.1f}cm", color='#7F8C8D', fontsize=8, ha='center', va='center')

        ax.set_xlim(-1, max(w_min + 3.0, 15.0))
        ax.set_ylim(-0.5, t + 0.8)
        ax.set_aspect('equal')
        ax.set_xlabel("Ancho Útil (cm)", fontsize=8)
        ax.set_ylabel("Espesor (cm)", fontsize=8)
...
if sol:
    ...
    st.markdown("---")
    st.subheader("📐 FASES 1 Y 2: Diagrama de Aserrado Primario (Sierra Cinta)")
    st.pyplot(generar_grafico_cortes(sol, d_menor))

    st.markdown("---")
    st.subheader("⚡ FASE 3: Esquema Visual de Piezas Efectivas en Sierra Múltiple")
    st.markdown("Disposición a escala de las piezas finales ($2\\times7$, $2\\times9$, $4\\times7$, $4\\times9$) canteadas y re-aserradas a partir de la madera de la sierra cinta:")
    st.pyplot(generar_grafico_fase3_multiples(sol))
    ...
    # --- FASE 3: PROGRAMACIÓN DE SIERRA MÚLTIPLE ---
    st.markdown("---")
    st.markdown("### ⚡ Detalle Operativo de Sierra Múltiple / Canteadora")
